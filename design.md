...
| FR-02 | データ蓄積・管理 | タイムスタンプ付きで時系列データをMySQLに保存する |
| FR-03 | 閾値異常検知 | 下限値 ≤ 計測値 ≤ 上限値 を外れた時点で即時アラートを発報する |
| FR-04 | 傾向異常検知 | UCL・CL・LCLを用いたトレンド管理で傾向異常を検知する（ON/OFF切替可能） |
...
| NFR-03 | 設定変更容易性 | 閾値・管理限界値はMySQLで管理し，Web UIから変更可能とする |
...
## 6．マスタ管理項目
以下の設定値はすべてMySQLで管理し，プログラム内にハードコーディングしない．
...
| 内部ポート | FastAPI :8000，MySQL :3306（外部非公開） |
